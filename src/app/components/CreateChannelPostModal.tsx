...
export function CreateChannelPostModal({
  isOpen, onClose, communityId, channelId, channelName, onPostCreated,
}: Props) {
  const { user } = useAuth();
  ...
  const handleFile = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALLOWED.includes(file.type)) {
      toast.error(`Format non autorisé (${file.type}). Acceptés : JPG, PNG, WEBP.`, { duration: 4000 });
      if (fileRef.current) fileRef.current.value = "";
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      toast.error(`Image trop volumineuse (${(file.size/1024/1024).toFixed(1)} MB). Max : 5 MB.`, { duration: 4000 });
      if (fileRef.current) fileRef.current.value = "";
      return;
    }
    setUploading(true);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `${BASE}/communities/${communityId}/channels/${channelId}/posts`,
        {
          method: "POST",
          headers: H,
          body: JSON.stringify({
      ...
      console.error("Erreur soumission post canal:", err);
          ...
          >
            ...
            <div style={{
              ...
              <div>
                ...
                <p style={{ fontSize: 12, color: "rgba(255,255,255,0.28)", margin: "2px 0 0" }}>
                  #{channelName}
                </p>
              </div>
              <motion.button
            ...
            <div style={{ padding: "16px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
                        ...
                        {BADGES.map((b) => (
                          <motion.button
                            ...
                            style={{
                              ...
                              border: badge === b.key ? "0.5px solid rgba(99,102,241,0.40)" : "0.5px solid rgba(255,255,255,0.09)",
                              cursor: "pointer",
                              fontSize: 13, fontWeight: badge === b.key ? 700 : 500,
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  <p style={{ fontSize: 14, fontWeight: 700, color: "rgba(255,255,255,0.88)", margin: 0 }}>
              ...
              <textarea
                ...
                placeholder={`Partage quelque chose dans #${channelName}…`}
                rows={5}
                style={{
                  width: "100%",
                  background: "rgba(255,255,255,0.04)",
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <motion.button
                    ...
                    style={{
                      ...
                      background: hashtag.trim() && hashtags.length < 5
                        ? "rgba(99,102,241,0.20)" : "rgba(255,255,255,0.04)",
                      color: hashtag.trim() && hashtags.length < 5
                        ? "#818cf8" : "rgba(255,255,255,0.20)",
            ...
            <div style={{
              ...
              <motion.button
                ...
                style={{
                  ...
                  border: imageUrl ? "0.5px solid rgba(99,102,241,0.30)" : "0.5px solid rgba(255,255,255,0.10)",
              ...
              <motion.button
                ...
                style={{
                  ...
                  boxShadow: !text.trim() || submitting ? "none" : "0 4px 16px rgba(79,70,229,0.35)",
                  cursor: !text.trim() || submitting ? "default" : "pointer",
                  fontSize: 15, fontWeight: 700,
                  color: !text.trim() || submitting ? "rgba(165,180,252,0.45)" : "#fff",
